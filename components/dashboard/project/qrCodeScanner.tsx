...
export default function QRCodeScanner({ projectId }: { projectId: string }) {
...
function ConnectionPanel({ projectId }: { projectId: string }) {
  ...
  const qrImage = !unavailable ? normalizeQrImage(data?.qrCodeBase64) : null;
  ...
  async function requestPairingCode() {
    ...
    try {
      ...
      const normalized = result.pairingCode.replace(/[\s-]/g, "").toUpperCase();
      ...
      if (mounted.current) {
        setActionError("La demande n’a pas abouti. Vous pouvez réessayer.");
      }
    } finally {
  ...
  if (connected) {
    return (
      ...
      >
        <div className="mx-auto mb-5 flex size-16 items-center justify-center rounded-2xl border border-border/20 bg-primary/10">
          <CheckCircle2
            aria-hidden="true"
        ...
        <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-muted-foreground">
          L’instance de votre projet est prête. Votre espace de conversation va
    ...
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
              Choisissez une méthode, puis validez la connexion depuis votre
              téléphone.
            </p>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-[1fr_1.05fr]">
        <div className="space-y-6 border-b border-border/70 bg-muted/20 p-5 sm:p-7 md:border-b-0 md:border-r">
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">
            Le nom des options peut varier selon votre version de WhatsApp.
          </p>
        </div>
          ...
          {unavailable ? (
            <div className="w-full max-w-sm text-center">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                Nous tentons de récupérer l’état de l’instance. Vous pouvez
                aussi relancer la vérification.
              </p>
            ...
            <>
              ...
              <p className="mt-2 max-w-xs text-center text-xs leading-relaxed text-muted-foreground">
                L’affichage se met à jour automatiquement lorsque le serveur
                fournit un nouveau QR code.
              </p>
            </>
          ) : (
            <div className="w-full max-w-sm">
              <div className="mb-5 text-center">
                ...
                <h3 className="text-base font-semibold">Connexion par code</h3>

                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  Le code sera associé au numéro WhatsApp configuré pour ce
                  projet.
                </p>
              </div>
      ...
      <footer className="border-t border-border/70 px-5 py-4 sm:px-7">
        <p className="text-xs leading-relaxed text-muted-foreground">
          ...
          Le QR code et le code de liaison permettent d’associer un appareil à
          votre compte. Ne les partagez pas, même avec une personne se
          présentant comme le support.
        </p>
      </footer>
    </section>
  );
