...
export default function TeamManagement({
  ...
  const getRoleBadge = (role: Role) => {
    switch (role) {
      case "OWNER":
        return (
          <Badge className="bg-primary/10 text-foreground dark:text-foreground hover:bg-primary/20 border-border/20 font-medium">
  ...
  const onSubmitInvite = (formData: InviteFormValues) => {
    // Renommé formData pour éviter un conflit de nom avec les props `data`
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Gérez les membres de votre projet, leurs rôles et leurs permissions
            d&apos;accès au CRM WhatsApp.
          </p>
        </div>
      ...
      <Alert className="border-border bg-muted/30">
        ...
        <AlertTitle className="font-semibold">
          Politique de sécurité extravertyAI
        </AlertTitle>
        <AlertDescription className="text-muted-foreground leading-relaxed mt-1">
          Les membres avec le rôle <strong>ADMIN</strong> ou{" "}
          <strong>OWNER</strong> peuvent modifier les paramètres du projet,
          récupérer les données et accéder aux informations de facturation
          Stripe. Assignez ces rôles avec précaution.
        </AlertDescription>
      </Alert>
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-0.5">
                        <span className="flex shrink-0 items-center gap-1">
                          <StatusIndicator
                            color={
                              member.status === "Online" ? "emerald" : "amber"
                            }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cette action révoquera immédiatement tous les accès de{" "}
              <strong>{memberToDelete?.email}</strong> à ce projet CRM WhatsApp.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <ChangeRoleModal
        ...
        onSuccess={(selectedRole: Role) => {
          ...
          setMembers((prev) =>
            prev.map((m) =>
              m.id === memberToEditRole?.id ? { ...m, role: selectedRole } : m,
            ),
          );
          // On s'assure que le serveur est bien synchro
          router.refresh();
